...
"""
 Prompt 模板
============================================
集中管理全系统 Prompt（供 llm / agent / services 引用）：

  - RAG 问答模板（build_rag_prompt + RAG_*）：把检索到的上下文 + 用户问题
    拼成完整 Prompt，/rag/search 调试与知识库问答使用；
...
# ------------------------------------------------------------
# 意图识别系统提示词：告诉模型什么时候调用工具、如何填参。
# 强化约束：只做路由，绝不回答用户问题（调研方案「router only」）。
# ------------------------------------------------------------
