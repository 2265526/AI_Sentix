"""
检索参数归一化层（LLM 输出 → 可执行检索参数）

LLM 输出的 arguments 是「不可信的原始值」（模型常把整句自然语言塞进 product_name、
用字符串表达价格区间），本层负责：
  1. 清洗意图词 / 疑问词 / 语气词，提取核心商品实体（品牌 / 型号 / 品类）；
  2. 文本价格 → (min, max) 数值；
  3. 统一输出规范化后的检索参数 dict，供 router 执行 SQL / RAG。

设计原则：LLM 输出永不直通 SQL；清洗/校验失败回退原始 query；检索永远有降级路径。
"""
import re
...
_INTENT_STOP = frozenset(
    ...
    库存 价格 售价 定价 报价 单价 多少 什么 哪几款 有哪些 多少钱一个
    便宜吗 贵不贵 贵吗 要求 需要 希望 最好 品牌 内存 够大 型号 参数 一款 是
    """.split()
)

# 单字意图/语气词：仅独立成词时删除（\b 边界；Python re 的 \w 含中文，
# '台灯' 中 '台' 前后是汉字非边界 → 不会误删；'看台' 同理安全）
_SINGLE_STOP = "台条件款和有要会能点个只吗么呢吧啊呀哦嘛哈哇啦的了货还买送是"
