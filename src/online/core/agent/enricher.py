# -*- coding: utf-8 -*-
"""
core/agent/enricher.py —— 查询增强（短期记忆驱动的指代/品牌/价格补全）
========================================================================
V2.2.0 短期记忆：基于 session_context 快照对用户问题做轻量增强，
把省略/指代/价格倾向补全为完整检索式，再交给意图识别。

4 条规则（按优先级，第一条命中即返回）：
  ① 切换意图检测：'不要手机/换电脑/看一下冰箱' → 原句直传并标记 switch_intent（上游清空会话）；
  ② 强指代消解：'它/这个/还有/再推荐' 用上一轮关键词补全（如 '还有便宜点的吗' → '手机 便宜点的'）；
  ③ 品牌品类补全：短句 + 品牌词 → '品牌 + 上一轮小类'；
  ④ 价格补全：含价格倾向词时按上一轮价格区间缩放（便宜→上限减半、贵→下限=0.7*上限）。

门控：
  - ctx.confidence 中低于 0.6 的实体不参与增强；
  - 原问题已完整（长度>=5 且含动词+名词）不增强；
