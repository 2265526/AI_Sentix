...
<!-- ================= V2.2.1 ================= -->
<details>
<summary>🏷️ V2.2.1 — Agent 架构优化与稳定性修复（点击展开）</summary>

#### 🔧 更新内容
- **Agent 架构优化**（ 参数归一化、多级降级链、意图预分类限制工具集）
- **短期记忆修复**：完整查询不套用旧记忆；记忆关键词清洗疑问词；记忆表 SELECT 失败回滚
- **检索降级标注**：品牌/类目/型号近似命中时结果明确提示「近似匹配」
</details>

<!-- ================= V2.2.2 ================= -->
<details>
<summary>🏷️ V2.2.2 — 请求级监控中心（点击展开）</summary>

#### 🔧 更新内容
- **导航栏新增「监控」页**：概览统计卡片（总请求 / 错误 / 降级 / 决策兜底 / LLM 失败 / 平均耗时 / token消耗）+ 最近请求表格 + 请求详情全链路时间线
- **全链路埋点**：每轮请求记录 ①记忆读取 → ②问题增强 → ③意图识别 → ④工具执行 → ⑤记忆回写 → ⑥⑦二次回调 六个阶段的各记录耗时+状态+参数摘要，错误标红带详情
- **问题定位**：出错时点击请求行，直接看到哪一步失败、参数摘要、是否走决策兜底/降级、LLM 是否失败

</details>

<!-- ================= V2.2.3 ================= -->
<details>
<summary>🏷️ V2.2.3 — 类目管理（点击展开）</summary>

#### 🔧 更新内容

- **管理员页新增「类目管理」Tab**：支持平台类目（大类 → 中类 → 小类 三级）的维护
- **新增类目**：可选大类 / 中类 / 小类；新增中类必须先选所属大类，新增小类必须先选所属大类与中类；同级下名称自动去重，完整 path（如 `服装鞋包/女装/衬衫`）自动拼接
- **类目搜索**：输入关键词可模糊/相似搜索类目名称或完整路径（如搜"衬衫"同时命中 `服装鞋包/女装/衬衫` 与 `服装鞋包/男装/衬衫`），结果展示层级与完整路径
